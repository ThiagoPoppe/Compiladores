...
void Montador::mount() {
    // Primeira linha será o tamanho do programa
    // e o número de labels que ele define
    std::cout << this->program.size() << " " << this->labels.size() << std::endl;

    // As próximas N linhas serão as labels que o programa define
    for (auto l : this->labels)
        std::cout << l.first << " " << l.second << std::endl;

    // O restante do arquivo será o assembly montado
