...
void CgenClassTable::code()
{
  ...
  // 1. prototype objects
  if (cgen_debug) cout << "emitting prototype objects" << endl;
  emit_prototype_objects();

  // 2. class_nameTab
  if (cgen_debug) cout << "emitting class_nameTab" << endl;
  emit_class_nameTab();

  // 3. class_objTab
  if (cgen_debug) cout << "emitting class_objTab" << endl;
  emit_class_objTab();

  // 4. dispatch tables
  if (cgen_debug) cout << "emitting dispatch tables" << endl;
...
/**** MY METHODS IMPLEMENTATION ****/

List<CgenNode>* CgenClassTable::reverse_list(List<CgenNode> *l) {
  List<CgenNode> *reversed = NULL;
  for ( ; l != NULL; l = l->tl())
    reversed = new List<CgenNode>(l->hd(), reversed);
...
int CgenClassTable::get_number_of_attributes(CgenNode *node) {
  // If node is null we must return 0
  if (node == NULL)
    return 0;

  // First we must compute object size for the class parent
  int n_attrib = get_number_of_attributes(node->get_parentnd());
...
void CgenClassTable::code_prototype_attributes(CgenNode *node) {
  // If node is null we must return
  if (node == NULL)
    return ;

  // First we need to code the attributes of our parent
  code_prototype_attributes(node->get_parentnd());
...
void CgenClassTable::emit_prototype_objects() {
  List<CgenNode> *reversed = reverse_list(nds);
  ...
  while (reversed != NULL) {
    ...
    if (cgen_debug) cout << "Gerando prototype para " << name << endl;

    str << WORD << -1 << endl; // .word -1
    str << name << PROTOBJ_SUFFIX << LABEL; // <class>_protoObj
    str << WORD << get_class_tag(name) << endl; // .word class_tag

    int n_attr = get_number_of_attributes(node);
    str << WORD << n_attr + 3 << endl; // .word object_size

    str << WORD << name << DISPTAB_SUFFIX << endl; // .word <class>_dispTable

    code_prototype_attributes(node);
...
void CgenClassTable::emit_dispTab(CgenNode *node) {
  // If node is null we must return
  if (node == NULL)
    return ;

  // First we need to emit the dispTab of our parent
  emit_dispTab(node->get_parentnd());
